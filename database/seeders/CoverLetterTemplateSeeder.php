<?php

namespace Database\Seeders;

use App\Models\CoverLetterTemplate;
use Illuminate\Database\Seeder;

class CoverLetterTemplateSeeder extends Seeder
{
    public function run(): void
    {
        $templates = [
            [
                'name' => 'ats-classic',
                'preview' => 'images/templates/ats-classic.png',
                'description' => 'ATS-friendly cover letter matching the ATS Classic CV style, with English and Arabic support.',
                'is_active' => true,
                'is_default' => true,
            ],
            [
                'name' => 'professional',
                'preview' => 'images/templates/professional.png',
                'description' => 'Professional cover letter layout with clean typography and bilingual support.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'serif-formal',
                'preview' => 'images/templates/serif-formal.png',
                'description' => 'Traditional serif letterhead with centered name and double-rule divider.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'stripe-modern',
                'preview' => 'images/templates/stripe-modern.png',
                'description' => 'Modern layout with a teal vertical brand stripe and clean sans body.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'editorial-masthead',
                'preview' => 'images/templates/editorial-masthead.png',
                'description' => 'Newspaper-style oversized masthead with editorial typography.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'mono-tech',
                'preview' => 'images/templates/mono-tech.png',
                'description' => 'Developer-focused monospace letter with a left meta stack.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'dual-tone',
                'preview' => 'images/templates/dual-tone.png',
                'description' => 'Two-tone navy and soft-blue header band with white name treatment.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'elegant-rules',
                'preview' => 'images/templates/elegant-rules.png',
                'description' => 'Quiet luxury letter with wide margins, hairline rules, and small-caps labels.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'banner-bold',
                'preview' => 'images/templates/banner-bold.png',
                'description' => 'Full-width charcoal name banner with high-contrast contact row.',
                'is_active' => true,
                'is_default' => false,
            ],
            [
                'name' => 'compact-exec',
                'preview' => 'images/templates/compact-exec.png',
                'description' => 'Dense executive letterhead with right-aligned date and corporate blue accents.',
                'is_active' => true,
                'is_default' => false,
            ],
        ];

        foreach ($templates as $template) {
            $template['preview_ar'] = preg_replace('/\.png$/', '-ar.png', $template['preview']);

            // Insert missing templates only — never overwrite existing rows.
            $record = CoverLetterTemplate::firstOrCreate(
                ['name' => $template['name']],
                $template,
            );

            if (! $record->preview_ar) {
                $record->update(['preview_ar' => $template['preview_ar']]);
            }
        }
    }
}
